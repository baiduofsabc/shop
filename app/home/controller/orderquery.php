...
/** 请填写以下配置信息 */
$mchid = '1545738121';          //微信支付商户号 PartnerID 通过微信支付商户资料审核后邮件发送
$appid = 'wx4b4f4e0b100c026d';  //公众号APPID 通过微信支付商户资料审核后邮件发送
$apiKey = '69ac4576eea9adf8fe362e613cda7986';   //https://pay.weixin.qq.com 帐户设置-安全设置-API安全-API密钥-设置API密钥
$outTradeNo = session('dingdanhao');     //要查询的订单号
// $outTradeNo = $info['dingdanhao'];     //要查询的订单号
/** 配置结束 */
...
if ($result['msg'] == '支付成功') {
    ...
    $info1 = db('users_card')->where('user_id', session('user.id'))->update(['integral' => $jifen, 'updatetime' => time()]);
    $info2 = db('users_card_follow')->insert(['user_id' => session('user.id'), 'integral' => $jifentmp['integral'], 'type' => 1, 'info' => '充值积分', 'updatetime' => time(), 'addtime' => time()]);
    ...
    // session('dingdanhao', null);
    // session('money', null);
    // echo '<script type="text/javascript">alert("充值成功'.$jifentmp['integral'].'|'.$jifentmp2['integral'].'|'.$jifen.'|'.$result['money'].'|'.'");window.location.href="http://fszbs.wanzhong666.com/home-User-setting.html"</script>';
}else{
    echo '<script type="text/javascript">alert("充值失败'.$result['msg'].'");window.location.href="http://fszbs.wanzhong666.com/home-User-setting.html"</script>';
}
