...
class Users extends Common{
    ...
    public function index(){
        if(request()->isPost()){
            ...
            $list=db('users')->alias('u')
                ->join(config('database.prefix').'users_card ul','u.id = ul.user_id','left')
                ->field('u.*,ul.integral')
    ...
    public function edit($id=''){
        if(request()->isPost()){
            ...
            if(!empty($data['addint'])) {

                $data2['integral'] = intval($data['addint']);
                ...
                $data3['integral'] = $data['integral'] + $data2['integral'];
             
                db('users_card_follow')->insert($data2);
                db('users_card')->where('user_id',$data['id'])->update($data3);
            }
            ...
            $info['integral'] = db('users_card')->where('user_id',$info['id'])->find()['integral'];
