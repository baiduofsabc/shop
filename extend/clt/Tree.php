...
class Tree {
    ...
    function get_nav($bid,$maxlevel,$effected_id='navlist',$style='filetree ' ,$homefont='',$recursion=FALSE ,$child='',$showenname='',$site_url='') {
        ...
        if($homefont) $homefont='<li id="nav_0"><span class="fl_ico"></span><a href-data="'.$site_url.'"><span class="fl">'.$homefont.'</span>'.$indexen.'</a></li>';
        $number=1;
        ...
        foreach($child as $id=>$a) {
            ...
            if($ischild && $level < $this->level){
                $this->ret .= '<span class="fd_ico"></span><a href-data="'.$url.'"><span class="fd">'.$catname.'</span>'.$enzm.'</a>';
                $this->get_nav($id,$maxlevel,$effected_id,$style,'',TRUE,$ischild,$showenname);
            } else {
                $this->ret .= '<span class="fl_ico"></span><a href-data="'.$url.'"><span class="fl">'.$catname.'</span>'.$enzm.'</a>';
            }
