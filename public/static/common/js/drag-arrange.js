...
(function (factory) {
  ...
  //extend_css 扩展自定义样式
  //callBack	拖拽时触发回调事件
  $.fn.arrangeable = function(options,extend_css,callBack) {
    var dragging = false;
    var $clone;
    var dragElement;
    ...
    function dragMoveHandler(e) {
      ...
      // check for drag threshold (drag has not started yet)
        ...
        $clone = clone($dragElement,extend_css);
  ...
  function clone($element,extend_css) {
    var $clone = $element.clone();
    if(extend_css !=undefined){
        $clone.css(extend_css);
    }
        $clone.css({
          position: 'absolute',
          width: $element.width(),
